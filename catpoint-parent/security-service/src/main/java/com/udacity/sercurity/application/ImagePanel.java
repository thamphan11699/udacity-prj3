...
/**
 * Panel containing the 'camera' output. Allows users to 'refresh' the camera by uploading their own
 * picture, and 'scan' the picture, sending it for image analysis
 */
public class ImagePanel extends JPanel implements StatusListener {
  ...
  public ImagePanel(SecurityService securityService) {
    ...
    addPictureButton.addActionListener(
        e -> {
          ...
          try {
            ...
            cameraLabel.setIcon(
                new ImageIcon(
                    tmp.getScaledInstance(IMAGE_WIDTH, IMAGE_HEIGHT, Image.SCALE_SMOOTH)));
