...
import com.udacity.sercurity.data.SensorType;
...
/**
 * Panel that allows users to add sensors to their system. Sensors may be manually set to "active"
 * and "inactive" to test the system.
 */
public class SensorPanel extends JPanel {
        ...
        e ->
            addSensor(
                new Sensor(
                    ...
                    SensorType.valueOf(
                        Objects.requireNonNull(newSensorTypeDropdown.getSelectedItem())
                            .toString()))));
  ...
  /**
   * Asks the securityService to change a sensor activation status and then rebuilds the current
   * sensor list
   *
  ...
  private void addSensor(Sensor sensor) {
    if (securityService.getSensors().size() < 4) {
      ...
      JOptionPane.showMessageDialog(
          null, "To add more than 4 sensors, please subscribe to our Premium Membership!");
    }
  }
